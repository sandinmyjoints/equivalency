...
/**
 * FunctionRule instances allow rules to be created with custom complex string transformations. When evaluating
 * equivalency, the supplied function is run against the two comparison strings. FunctionRules are only run after
 * all MapRules have been processed.
 * @param {function} fn - callable that takes two strings as its arguments and applies custom transformations to them
 * @param {object} opts
 */
...
/**
 * MapRule instances allow rules to be created that are described by a custom map. When evaluating equivalency, the
 * supplied map is used to replace characters in the two comparison strings. In a rule chain with multiple map rules,
 * all MapRule maps are collapsed into a single map which is applied to the strings in a single operation.
 * All MapRules are processed before any FunctionRules.
 * @param {Map} map - mapping where the keys are source character(s) and the values are target character(s) for replacement
 * @param {object} opts
 */
...
function ReplaceRule(str, replaceStr, opts = { modifiesCharacters: true }) {
  assert(
    typeof str === 'string',
    `${str} is not a string (is ${str.constructor})`
  );
  const map = mapFromString(str, replaceStr);
  MapRule.call(this, map, opts);
  this.type = 'ReplaceRule';
}
inherits(ReplaceRule, MapRule);
...
const ZERO_WIDTH_RE = /[\u200B]/g; // zero-width space
const whitespaceDifferences = (s1, s2) => {
  return [
    s1
      .trim()
      .replace(RE_ANY_WHITESPACE, ' ')
...
const codepointRuleGenerator = (codepoints, name) => {
  return new FunctionRule(
    (s1, s2) => {
      ...
      s1prime = replaceAllCodepoints(s1prime, codepoints);
      s2prime = replaceAllCodepoints(s2prime, codepoints);
