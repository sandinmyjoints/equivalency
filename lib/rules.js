...
/**
 * Creates a function rule from an input string of escaped punctuation symbols that replaces symbols with a whitespace
 * @param punctuationSymbols (ES and EN have different punctuation symbols)
 * @returns {FunctionRule}
 */
const punctuationAsWhitespaceFactory = (punctuationSymbols) => {
  const RE_PUNCTUATION = RegExp('\\s?[' + punctuationSymbols + ']\\s?', 'g');
  return new FunctionRule(
    (s1, s2) => {
      return [s1.replace(RE_PUNCTUATION, ' ').trim(), s2.replace(RE_PUNCTUATION, ' ').trim()];
    },
    { name: 'punctuationAsWhitespace'}
  );
};
...
module.exports = {
  ...
  punctuationAsWhitespaceFactory
};
