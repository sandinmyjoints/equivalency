...
const punctuationAsWhitespace = (s1, s2) => {
  const RE_PUNCTUATION = /[!"',\-.:;?`…“”‘’$#&%()*+/<=>@/[\]^_{|}~¿¡«»€]\s?/g;
  return [s1.replace(RE_PUNCTUATION, ' ').trim(), s2.replace(RE_PUNCTUATION, ' ').trim()];
};
const punctuationAsWhitespaceRule = new FunctionRule(punctuationAsWhitespace, {
...
module.exports = {
  ...
  PUNCTUATION_AS_WHITESPACE: punctuationAsWhitespaceRule
};
