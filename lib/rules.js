...
const nTildeRule = new FunctionRule((s1, s2) => {
  let s1prime = normalize(s1, 'NFD');
  let s2prime = normalize(s2, 'NFD');
...
const combiningDiacriticsBlockExceptAcuteAndUmlautAndNTildeRule = new FunctionRule((s1, s2) => {
  let s1prime = normalize(s1, 'NFD');
  let s2prime = normalize(s2, 'NFD');
  ...
  [
    ...
    s1prime = replaceAll(s1prime, codepoint);
    s2prime = replaceAll(s2prime, codepoint);
  });
